...
class BasicEmbedding(object):
    ...
    If building a model that uses a concatenation of traditional word
    embeddings with those generated by char2vec then it can be useful to use
    dropout on the traditional embeddings to avoid favoring them too much
...
class Char2Vec(object):
    ...
    def __init__(self, char_vocab, max_sequence_len=15):
        ...
        self.words_as_chars = tf.placeholder(tf.int32,
    ...
    def MakeMat(self, word_list, pad_len=None):
        ...
        Returns:
          Array containing character sequences and a vector of sequence
          lengths.
        """
        if not pad_len:
...
class CharLSTM(Char2Vec):
    ...
    def __init__(self, char_vocab, model_params,
        ...
        with tf.variable_scope('c2v'):
            ...
            layer1_fw = GetCell(layer1_hidden_size, layer1_out_size,
                                model_params['peepholes'])
            layer1_bw = GetCell(layer1_hidden_size, layer1_out_size,
                                model_params['peepholes'])

            # This is the 2nd layer, also a bi-LSTM. The input size is twice
            # the size of the output size from layer one because the
            # concatenation of the layer one outputs is the layer 2 input.
            layer2_fw = LSTMCell(self.hidden_size,
                                 use_peepholes=model_params['peepholes'])
            layer2_bw = LSTMCell(self.hidden_size,
                                 use_peepholes=model_params['peepholes'])

            # The final embeddings is the output from the layer2 LSTM
            # multiplied by this matrix. One purpose of this matrix is to scale
            # the layer2 output to match the specified number of dimentions for
            # the word embedding.
            out_mat = tf.get_variable('out_mat',
                                      [2 * self.hidden_size, word_embed_dims],
                                      initializer=initializer)

            # z should be a tensor of dimensions batch_sz x word_len x
            # embed_dims.
            ...
            # There is one entry per timestep and one character is processed
            # per timestep.
            inputs = [tf.squeeze(input_) for input_ in
                      tf.split(1, max_sequence_len, z)]
            # newest version of tf needs help with shape inference
            for _i in inputs:
            ...
            out1, _, _ = tf.nn.rnn.bidirectional_rnn(layer1_fw, layer1_bw,
            ...
            # direction. It is a little tricky to grab these states because
            # they are in different positions for each word in the minibatch.
            batch_range = tf.range(self.batch_dim)
            # The slices variables keeps track of which position the
            # appropriate outputs are for each word in the minibatch.
            slices = self.batch_dim * tf.to_int32(self.seq_lens-1) + batch_range
            with tf.variable_scope('fw'):
                outputs_forward, _ = tf.nn.rnn.rnn(layer2_fw, out1,
                                                   dtype=tf.float32)
                out_forward = self._GetLastOutput(outputs_forward, slices)
            with tf.variable_scope('bw'):
                # Reverse the sequences before processing with the backwards
                # LSTM.
                out1_bw = reverse_seq(out1, self.seq_lens)
                outputs_backward, _ = tf.nn.rnn.rnn(layer2_bw, out1_bw,
                                                    dtype=tf.float32)
    ...
    def __init__(self, char_vocab, model_params,
        ...
        with tf.variable_scope('c2v'):
            ...
            self.embedding = tf.get_variable("embedding",
                                             [self._vocab_size, char_embed_dims],
                                             initializer=initializer)
            the_filter, filter_b = MakeFilter(3, char_embed_dims,
                                              layer1_out_size, 'filt')
            ...
            if dropout_keep_prob is not None:
                h_expanded = tf.nn.dropout(tf.expand_dims(h, -1),
                                           dropout_keep_prob)
            else:
            ...
            for width in filter_sizes:
                ...
                pooled = tf.nn.max_pool(h2, ksize=[1, max_sequence_len-1-width,
                                                   1, 1],
