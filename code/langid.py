...
from char2vec import CharCNN as Char2Vec, BasicEmbedding
...
def Eval(expdir):
    ...
    for _ in range(dataset.GetNumBatches()):
        ...
        idx = weights != 0
        all_preds += [output_vocab[p]
                      for p in np.argmax(probs[idx, :], axis=1)]
        all_labs += [output_vocab[p]
                     for p in np.argmax(labs[idx, :], axis=1)]
    ...
    for idx in range(maxitr):
        ...
        if idx % 25 == 0:
            ...
            if args.data != 'codeswitch':
                ...
                print("Predict:", GetTopWordLevelPreds(probs[0, :, :],
                                                       seqlen[0]))
                print("Actual: ", GetTopWordLevelPreds(labs[0, :, :],
                                                       seqlen[0]))
