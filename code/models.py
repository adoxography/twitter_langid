...
class BaseModel(object):
    ...
    def _DoPredictions(self, in_size, mats, class_weights=None):
        ...
        self.cs = old_div(self._mask,
                          tf.reduce_sum(self._mask, 1, keepdims=True))

        # The final prediction is the average of the predictions for each word
        # weighted by the individual confidence/utility scores.
        preds_weighted = tf.multiply(tf.reshape(tf.transpose(self.cs),
                                     [-1, 1]),
...
class WordSeqModel(BaseModel):
    ...
    def __init__(self, out_vocab_size=None,
        ...
        z = tf.tile(tf.reshape(tf.reduce_sum(preds, 1), [-1, 1]),
                    [1, out_vocab_size])
...
class TweetSeqModel(BaseModel):
    ...
    def __init__(self, out_vocab_size=None,
        ...
        with tf.variable_scope('wordrnn'):
            ...
            super(TweetSeqModel, self)._DoPredictions(out_size, rnnout,
                                                      class_weights=weights)

            self.cost = tf.reduce_mean(self.example_weights * self._xent)


class CharSeqModel(object):
    """
...
class WordLevelModel(object):
    ...
    def __init__(self, batch_size, model_params, c2v, max_sequence_len,
        ...
        # Use the logical operations to create a mask, i.e. where seq len is
        # less than index
        indicator = tf.less(range_tiled, lengths_tiled+1)
        ...
        with tf.variable_scope('wordrnn'):
            ...
            rnnout, _, _ = tf.nn.bidirectional_rnn(cell_fw, cell_bw,
                                                   self._inputs,
                                                   dtype=tf.float32,
                                                   sequence_length=self.seq_lens)
            if proj_size:
